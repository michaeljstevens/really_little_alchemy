...
/***/ function(module, exports, __webpack_require__) {
	...
	let offset;
	let update = true;
	let initial = ["fire", "water", "earth", "air"];
	let all = ["fire", "water", "earth", "air", "pressure", "energy", "dust", "lava",
	...
	let winModal = new createjs.Shape();
	...
	let winModalLabel = new createjs.Text("You Win!", "80px Arial", "#000");
	winModalLabel.x = 490;
	winModalLabel.y = 190;
	...
	document.addEventListener("DOMContentLoaded", function() {
	  ...
	  line.graphics.endStroke();
		...
		let disContainer = new createjs.Container();
		...
		let aboutModal = new createjs.Shape();
		...
		let modalLabel = new createjs.Text("Really Little Alchemy", "40px Arial", "#000");
		modalLabel.x = 490;
		modalLabel.y = 120;
		...
		let modalDescription = new createjs.Text("", "20px Arial", "#000");
		...
		let buttonok = new createjs.Shape();
		...
		let buttonokLabel = new createjs.Text("Continue", "20px Arial", "#000");
		buttonokLabel.x = 640;
		buttonokLabel.y = 360;
		modalDescription.lineWidth = 300;
		modalDescription.lineHeight = 20;
	...
	function handleImageLoad(event) {
	  let image = event.target;
		...
		let text = new createjs.Text(this.name, "18px Arial", "#ff7700");
		...
		let foundCount = new createjs.Text(`${discovered.length}/100`, "72px Sans-Serif", "#ff7700");
	  ...
	  bitmap.on("pressup", function (evt) {
	      ...
	      for (let i = 0; i < elements.length; i++) {
	        ...
	        if (this !== element && !(element.x - 15 > this.x + 15 ||
					 ...
					 if([this.name, element.name].sort().join()===["atomic bomb", "earth"].join()) {
						 ...
						 let explode = new Audio('./assets/sounds/nuclear_war.mp3');
			       ...
			       imageDup.onload = handleImageLoad.bind(elObj);
						 const laugh = new Audio("./assets/sounds/great_job.wav");
						 if(!mute) laugh.play();
					 } else {
							let greatJob = new Audio("./assets/sounds/laugh.wav");
		...
		if (stage.children[1].children.length >= 100) {
			let cheer = new Audio('./assets/sounds/win.mp3');
			if(!mute) cheer.play();
